...
export function renderColumnaPagina() {
    ...
    <div style="display:grid; grid-template-columns:1fr 420px; gap:28px; max-width:1400px; margin:0 auto; align-items:start;">
        ...
        <div style="display:flex; flex-direction:column; gap:20px;">
            ...
            <div style="display:flex; align-items:center; gap:16px; padding-bottom:10px;">
                <button id="btn-guardar-pagina" onclick="window._paginaGuardar()"
                    class="pag-btn-guardar">
        ...
        <!-- ═══ COLUMNA DERECHA: Preview reactivo ═══ -->
        <div style="position:sticky; top:20px;">
            <div class="pag-card" style="padding:14px;">
                ...
                <div id="pag-preview-wrap" style="transform-origin:top left; transform:scale(0.55); width:182%; margin-bottom:-230px;">
                    <div id="pag-preview">${_renderPreviewCompleto(c)}</div>
                </div>
            </div>

            <!-- Panel de upload (aparece al hacer clic en una imagen) -->
            <div id="pag-upload-panel" style="display:none; margin-top:16px;">
                <div class="pag-card" style="border-color:#7b2fff;">
                    <div class="pag-card-title" style="color:#b07aff;">📤 Subir imagen: <span id="pag-upload-nombre" style="color:#fff;"></span></div>
                    <p id="pag-upload-zona-label" style="color:#666; font-size:0.75em; font-family:sans-serif; margin:4px 0 12px 0;"></p>
                    ...
                    <button onclick="window._paginaCerrarUpload()"
                        style="margin-top:10px; background:#1a0000; border:1px solid #4a1818; color:#aa4444;
                               padding:6px 14px; border-radius:4px; cursor:pointer; font-family:'Cinzel'; font-size:0.75em;">
                        Cancelar
                    </button>
                </div>
            </div>
    ...
    <style>
        ...
        .pag-img-card .pag-img-label { font-size:0.62em; color:#aaa; font-family:sans-serif; line-height:1.3; word-break:break-word; }
        .pag-img-card .pag-img-zona  { font-size:0.58em; color:#555; font-family:sans-serif; margin-top:3px; font-style:italic; }
        .pag-img-card .pag-upload-badge { position:absolute; top:4px; right:4px; background:#7b2fff; color:#fff; font-size:0.55em; padding:1px 5px; border-radius:3px; font-family:sans-serif; }
...
// ── Render de cada card de imagen ────────────────────────────
function _renderImgCard(img) {
    const url = `${STORAGE_URL}/imginterfaz/${img.key}.png?v=${Date.now()}`;
    const fallback = `${STORAGE_URL}/imginterfaz/no_encontrado.png`;
    const safeKey  = img.key.replace(/'/g, "\\'");
    const safeArch = img.archivo.replace(/'/g, "\\'");
    const safeLbl  = img.label.replace(/'/g, "\\'");
    const safeZona = img.zona.replace(/'/g, "\\'");

    return `
    <div class="pag-img-card" onclick="window._paginaAbrirUpload('${safeKey}','${safeArch}','${safeLbl}','${safeZona}')" title="${img.zona}">
        <span class="pag-upload-badge">📤</span>
        <img src="${url}" onerror="this.onerror=null;this.src='${fallback}'">
        <div class="pag-img-label">${img.label}</div>
        <div class="pag-img-zona">${img.zona}</div>
    </div>`;
}

// ── Preview completo del index ───────────────────────────────
function _renderPreviewCompleto(c) {
    const bgUrl   = `${STORAGE_URL}/imginterfaz/hex-002.png`;
    const iconUrl = `${STORAGE_URL}/imginterfaz/icon.png`;
    const storBase = STORAGE_URL;

    const gridItems = [
        { archivo:'objetos.jpg',     label:'OBJETOS' },
        { archivo:'estadisticas.jpg',label:'ESTADÍSTICAS' },
        { archivo:'misiones.png',    label:'MISIONES' },
        { archivo:'hechizos.jpg',    label:'HECHIZOS' },
        { archivo:'mapa.png',        label:'MAPA HECHIZOS' },
        { archivo:'extra.png',       label:'IMÁGENES' },
        { archivo:'region.png',      label:'MAPA REGIONAL' },
    ];

    return `
    <div style="background:#0a0012; min-height:600px; font-family:'Cinzel',serif; color:#d4af37; border-radius:10px; overflow:hidden; border:1px solid #2a0050;">
        ...
        <div style="background:#05000a; border-bottom:1px solid #2a0050; padding:8px 16px; display:flex; justify-content:space-between; align-items:center;">
            <img src="${iconUrl}" onerror="this.onerror=null" style="width:24px;height:24px;border-radius:4px;">
        ...
        <!-- Header -->
        <div style="background:linear-gradient(rgba(18,0,36,.7),rgba(18,0,36,.7)),url('${bgUrl}') center/cover; padding:28px 16px 20px 16px; text-align:center;">
            <h1 id="prev-titulo" style="font-size:1.3em; margin:0 0 6px 0; text-shadow:0 0 20px rgba(0,0,0,.9);">${_esc(c.titulo||'HEXCRAFT')}</h1>
            <p id="prev-subtitulo" style="color:#fff; letter-spacing:2px; font-size:0.5em; text-transform:uppercase; margin:0;">${_esc(c.subtitulo||'Campaña de rol')}</p>
        </div>

        <!-- Lore -->
        <div style="background:rgba(20,0,40,.5); border:1px solid #2a0050; border-radius:6px; padding:10px 14px; margin:10px 14px; font-size:0.5em; color:#bbb; font-family:sans-serif; font-style:italic; line-height:1.5;">
        ...
        <div style="padding:0 14px 10px 14px;">
            ...
            <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:6px;">
                ...
                <div style="background:rgba(10,0,20,.8); border:1px solid #2a0050; border-radius:5px; overflow:hidden; text-align:center; padding:4px;">
                    <img src="${storBase}/imginterfaz/${gi.archivo.replace(/\.(jpg|png)$/,'')}.png" 
                         onerror="this.onerror=null;this.src='${storBase}/imginterfaz/${gi.archivo}'" 
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px; padding:0 14px 14px 14px;">
            <div style="background:rgba(10,0,20,.8);border:1px solid #2a0050;border-radius:5px;padding:6px;text-align:center;">
                <div id="prev-nombre-rol" style="font-size:0.5em;color:#d4af37;">${_esc(c.nombre_rol||'ROL ACTUAL')}</div>
            </div>
            <div style="background:rgba(10,0,20,.8);border:1px solid #2a0050;border-radius:5px;padding:6px;text-align:center;">
                <div id="prev-nombre-meta" style="font-size:0.5em;color:#d4af37;">${_esc(c.nombre_meta||'META')}</div>
            </div>
        </div>
    </div>`;
}
...
function _exponerGlobales() {
    window._paginaMod = (campo, valor) => {
        ...
        // Actualizar preview reactivamente sin re-renderizar todo
        const ids = { titulo:'prev-titulo', subtitulo:'prev-subtitulo', lore:'prev-lore', nombre_rol:'prev-nombre-rol', nombre_meta:'prev-nombre-meta' };
    ...
    window._paginaGuardar = async () => {
        ...
        const status = document.getElementById('pag-status');
        if (!haycambiosPagina()) { if (status) status.textContent = '✓ Sin cambios'; return; }
    ...
    window._paginaAbrirUpload = (key, archivo, label, zona) => {
        ...
        document.getElementById('pag-upload-panel').scrollIntoView({ behavior:'smooth', block:'nearest' });
    };

    window._paginaCerrarUpload = () => {
        _uploadTarget = null;
        document.getElementById('pag-upload-panel').style.display = 'none';
        document.getElementById('pag-file-input').value = '';
    };
...
async function _ejecutarSubidaPagina(file) {
    ...
    const setProgreso = (pct, msg, color = '#888') => {
    ...
    try {
        setProgreso(20, 'Procesando imagen...');

        // Convertir a PNG y JPG
        ...
        setProgreso(100, '✅ ¡Imagen actualizada!', '#00ff88');

        // Refrescar la card en el grid añadiendo cache-bust
        const cardImg = document.querySelector(`.pag-img-card[onclick*="'${key}'"] img`);
        if (cardImg) cardImg.src = `${STORAGE_URL}/imginterfaz/${keyNorm}.png?v=${Date.now()}`;

        setTimeout(() => window._paginaCerrarUpload(), 1800);

    } catch(e) {
        setProgreso(0, '❌ ' + (e.message || 'Error al subir'), '#ff4444');
        ...
        img.onload = () => {
            try {
                ...
                c1.toBlob(blobPNG => {
                    ...
                    ctx2.fillStyle = '#05000a'; ctx2.fillRect(0,0,w,h);
