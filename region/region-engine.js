...
function dibujarCapaMid(propIds, cx, cy) {
    const size    = HEX_SIZE * camara.zoom;
    const offsetY = size * 0.10;   // Más centrado, menos salto
    ...
    if (limit === 1)      { itemSize = size * 1.5; radioCirculo = 0; }
    else if (limit === 2) { itemSize = size * 0.8; radioCirculo = size * 0.42; }
    else {
    ...
    for (let i = 0; i < limit; i++) {
        ...
        ctx.globalAlpha = 0.3;
        ctx.beginPath();
        ctx.ellipse(sx, sy + itemSize * 0.45 + offsetY * 0.5, itemSize * 0.4, itemSize * 0.15, 0, 0, Math.PI * 2);
...
function dibujarCapaOver(propIds, cx, cy) {
    const size    = HEX_SIZE * camara.zoom;
    const offsetY = size * 1.1; // Altura: 1 nivel de hex entero más arriba
    const overCy  = cy - offsetY;

    propIds.forEach(pid => {
        if (typeof pid === 'string' && pid.startsWith('COLOR:')) {
            const parts = pid.split(':');
            ctx.save();
            trazarHexPath(cx, overCy);
        ...
        // 1. Sombra base proyectada en el suelo
        ctx.save();
        ...
        // 2. Relleno texturizado (clipeado) al hexágono flotante superior
        ctx.save();
        trazarHexPath(cx, overCy);
        ctx.clip();
        const w = size * 2.2, h = w * 0.92;
        ctx.drawImage(img, cx - w / 2, overCy - h / 2, w, h);
        ctx.restore();

        // 3. Iluminación y Relieve 3D para el hexágono over
        trazarHexPath(cx, overCy);
        const grad = ctx.createLinearGradient(cx, overCy - size, cx, overCy + size);
        ...
        // 4. Borde inferior grueso para darle el grosor 3D flotante
        ctx.save();
        const verts = hexVertices(cx, overCy);
        ctx.beginPath();
        ctx.moveTo(verts[2].x, verts[2].y);
        ctx.lineTo(verts[3].x, verts[3].y);
        ctx.lineTo(verts[4].x, verts[4].y);
